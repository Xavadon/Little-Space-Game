...
public class BomUpgradedController : MonoBehaviour
{
    ...
    void Explode()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            bomb.GetComponent<BombController>().damage = damage;
            Vector3 randomDir = new Vector3(Random.Range(-1f, 1f), Random.Range(-1f, 1f));
            bomb.GetComponent<Rigidbody2D>().AddForce(randomDir.normalized * Random.Range(15f, 17f), ForceMode2D.Impulse);
        }
        foreach (Collider2D obj in objects)
        {
            ...
            if (isPlayerBomb)
            {
                ...
                {
                    obj.GetComponent<EnemyTurrelController>().DamageEnemy(100 + damage);
                }
            }
            if (obj.tag == "Player" && isEnemyBomb)
            {
                ...
                {
                    obj.GetComponent<PlayerController>().DamagePlayer(100 + damage);
                }
            }
